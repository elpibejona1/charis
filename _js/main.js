...
var mapInitial = function(){
	if ($(window).width() > 768) {
		var mapContTween = new TimelineMax;

		mapContTween.to($('.container--historyMap'), 1, { top: '50%' })
					.to($('.history__heading, .history__scroll'), 1, { opacity: 0 }, "-=1");
...
var advanceSlides = function(){
	...
	$('.timeline__trigger:nth-child(odd)').each(function(){
		

		if ( oddCounter > 1 ) {
			var previous = oddCounter - 1;
			var previousCountry = oddCounter - 2;

			var tweenOddSlide = new TimelineMax;

			tweenOddSlide.to($('.fullContainer--history--' + oddCounter), 0.25, { y: 0 })
					.to($('.fullContainer--history--' + previous), 0.25, { y: '-100%' }, "-=0.25")
					.to($('.histCountry[data-number="' + previousCountry + '"'), 1, { fill: "#696969" })
					.to($('.histCountry[data-number="' + oddCounter + '"'), 1, { fill: "#097888" }, "-=1")
					.to($('.history__wrapper'), 1, { height: '+=180px' })
		} else {
			var tweenOddSlide = new TimelineMax;

			tweenOddSlide.to($('.histCountry'), 0, { fill: "#e3e3e3" })
					.to($('.fullContainer--history--' + oddCounter), 0.25, { y: 0 })
					.to($('.histCountry[data-number="' + oddCounter + '"'), 1, { fill: "#097888" })
					.to($('.history__wrapper'), 1, { height: '+=180px' })
		}
	...
	$('.timeline__trigger:nth-child(even)').each(function(){
		var previous = evenCounter - 1;

		var tweenEvenSlide = new TimelineMax;

		tweenEvenSlide.to($('.fullContainer--history--' + evenCounter), 0.25, { y: 0 })
					.to($('.fullContainer--history--' + previous), 0.25, { y: '-100%' }, "-=0.25")
					.to($('.history__wrapper'), 1, { height: '+=180px' })
