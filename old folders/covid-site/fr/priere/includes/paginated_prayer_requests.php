<?php /* Prayer Engine - Paginates All Public Prayer Requests for the Prayer Wall */
	...
	$getall = "SELECT * FROM prayers WHERE post_this = 1 AND (share_option = 'Share Online' OR share_option = 'Share Online Anonymously') " . $enmpe_displaylimitsql . " ORDER BY id DESC"; //Get records from database
	...
	$findprayers = "SELECT * FROM prayers WHERE post_this = 1 AND (share_option = 'Share Online' OR share_option = 'Share Online Anonymously') " . $enmpe_displaylimitsql . " ORDER BY id DESC LIMIT $start, $display";	
